...
void bluetoothStart() {
    ...
    char name[255];
    snprintf(name, sizeof(name), "RaceChrono DIY #%2X%2X", mac[4], mac[5]);
...
void gpsSetup() {
    delay(2000);
    Serial.begin(9600);
    Serial.write(PMTK_SET_BAUD_115200); // Set baud rate to 115200
    Serial.write("\r\n");
    Serial.write("\r\n");
    Serial.end();
    delay(1000);
...
void gpsLoop() {
    ...
    do 
    {
        ...
        good_data = gps.encode(Serial.read());
    ...
    if (1) {
        ...
        int dateAndHour = ((gps.date.year()-2000) * 8928) + ((gps.date.month()-1) * 744) + ((gps.date.day()-1) * 24) + gps.time.hour();
        ...
        int timeSinceHourStart = (gps.time.minute() * 30000) + (gps.time.second() * 500) + (gps.time.centisecond() * 10 / 2);

        // Calculate latitude and longitude
        int latitude = int(gps.location.lat()*10000000); // / 10000000) * 6000000 + (gps->latitude_fixed % 10000000);
        int longitude = int(gps.location.lng()*10000000); // / 10000000) * 6000000 + (gps->longitude_fixed % 10000000);
        ...
        tempData[17] = bearing;
        tempData[18] = round(gps.hdop.hdop() * 10.f);
